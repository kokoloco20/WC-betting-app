  ...
  return (
    <div className="mx-auto max-w-xl px-4 pt-5 pb-28 lg:max-w-5xl">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div style={{ opacity: 'var(--block-opacity)' }}>
        ...
        {/* wash so content keeps contrast */}
      ...
      <header className="mb-5 flex items-center justify-between">
        <h1 className="flex items-center gap-2.5 text-lg font-extrabold tracking-tight">
          <span className="rounded-lg border border-black/10 bg-[#fff] px-1.5 py-0.5 text-base leading-none font-extrabold tracking-tighter text-[#0c0a12]">
            26
          </span>
          <span>
            World Cup
      ...
      <nav className="fixed inset-x-0 bottom-3 px-4">
        <div className="panel mx-auto flex max-w-md gap-1 rounded-2xl border p-1.5 shadow-xl shadow-black/50 backdrop-blur-md">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
